document.addEventListener('DOMContentLoaded', () => {
    ...
    function createRemoteVideoElement(peerId, stream) {
        ...
        try {
            ...
            // - обычные игроки видят череп если игрок жив или ангелочка если игрок убит, но некликабельную
            ...
            if (!isHost) {
                if (userRole === 'host') {
                    ...
                    // Для обычных игроков - показываем некликабельную кнопку
                    if (isKilled) {
                        // Ангелочка для убитых
                        killButtonHtml = `<button class="kill-button angel-icon" title="Вбито">👼</button>`;
                    } else {
                        // Череп для живых (но некликабельный)
                        killButtonHtml = `<button class="kill-button" title="Вбито">💀</button>`;
                    }
                }
            }
